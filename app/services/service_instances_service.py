...
class ServiceInstancesService:
    ...
    async def get_all_service_instances(cls, event_id: str) -> list[ServiceInstance]:
        ...
        _service_instances = await ServiceInstancesAdapter.get_all_service_instances(event_id)
        service_instances = [ServiceInstance.model_validate(e) for e in _service_instances]
        return sorted(
            service_instances,
            key=lambda k: (
    ...
    @classmethod
    async def get_service_instances_by_service_type(cls, event_id: str, service_type: str) -> list[ServiceInstance]:
        """Get all service instances for one service_type function."""
        _service_instances = await ServiceInstancesAdapter.get_service_instances_by_service_type(
    ...
    @classmethod
    async def get_service_instances_by_status(cls, event_id: str, status: str) -> list[ServiceInstance]:
        """Get all service instances for one status function."""
        _service_instances = await ServiceInstancesAdapter.get_service_instances_by_status(
            event_id, status
        )
        service_instances = [ServiceInstance.model_validate(e) for e in _service_instances]
        return sorted(
    ...
    @classmethod
    async def create_service_instance(cls, service_instance: ServiceInstance) -> str | None:
        ...
        result = await ServiceInstancesAdapter.create_service_instance(new_service_instance)
    ...
    async def get_service_instance_by_id(cls, c_id: str) -> ServiceInstance:
        ...
        service_instance = await ServiceInstancesAdapter.get_service_instance_by_id(c_id)
        if service_instance:
    ...
    @classmethod
    async def update_service_instance(cls, c_id: str, service_instance: ServiceInstance) -> str | None:
        """Update service instance function."""
        old_service_instance = await ServiceInstancesAdapter.get_service_instance_by_id(c_id)
        if old_service_instance:
            if service_instance.id != old_service_instance["id"]:
                err_msg = "Cannot change id for service instance."
                raise IllegalValueError(err_msg) from None
            new_service_instance = service_instance.model_dump()
            return await ServiceInstancesAdapter.update_service_instance(c_id, new_service_instance)
        err_msg = f"Service instance with id {c_id} not found."
        raise ServiceInstanceNotFoundError(err_msg) from None
